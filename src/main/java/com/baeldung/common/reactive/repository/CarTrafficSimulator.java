...
@Service
public class CarTrafficSimulator implements CarTraffic {

	private static Logger LOGGER = LoggerFactory.getLogger(CarTrafficSimulator.class);
	private static DecimalFormat plateFormatNumber = new DecimalFormat("0000");

	public CarTrafficSimulator() {
	}
	...
	public Flux<Vehicle> flowTraffic() {
		...
		return Flux.<Vehicle>create(fluxSink -> {
			...
			while (index <= 30000 && inFrameTime && !fluxSink.isCancelled() ) {
				fluxSink.next(CarUtilities.simulateTraffic());
				index++;
	...
	private static class CarUtilities {
